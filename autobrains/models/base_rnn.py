...
class BaseRNN3(nn.Module):
    def __init__(self, rnn_fn: nn.Module, config: dict) -> None:
        super(BaseRNN3, self).__init__()
        self.config = config
        ...
        self.fc_layers = nn.ModuleList(
            [
                get_sequential_mlp_with_last_layer(
                    input_size=self.config["hidden_size"],
                    units=[self.config["hidden_size"]],
                    activation=self.config["activation_fn"],
                    norm_func_name=self.config["speed_sequential_mlp"][
                        "norm_func_name"
                    ],
                    need_norm=self.config["speed_sequential_mlp"]["need_norm"],
                    last_layer=self.config["dim_waypoints"]
                )
                # nn.Linear(self.config["hidden_size"], self.config["dim_waypoints"])
                for _ in range(self.config["num_waypoints"])
            ]
